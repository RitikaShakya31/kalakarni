...
class CommonModal extends CI_Model
{
	...
	public function getRowByMoreIdInOrder($table, $where, $orderColumn, $type)
    {
    	$get = $this->db->select()
    		->from($table)
    		->where($where)
    		->order_by($orderColumn, $type)
    		->get();
    	if ($get->num_rows() > 0) {
    		return $get->result_array();
    	} else {
	...
	public function filterdata($minimum_price, $maximum_price, $search, $category, $fabric, $length, $pattern)
	{
		$sql = array();
		$price = array();
		$loca = array();
		if (($minimum_price == '') && ($maximum_price == '') && ($search == '') && ($category == '') && ($fabric == '') && ($length == '') && ($pattern == '')) {
			$query = "SELECT * FROM `tbl_products`";
		} else {
			$query = "SELECT * FROM `tbl_products` WHERE   `pro_name` IS NOT NULL AND ";

			if ($minimum_price != '' || $maximum_price != '') {
				if ($minimum_price != '' && $maximum_price != '') {
					$query .= "`price` BETWEEN " . (($minimum_price == '') ? '0' : $minimum_price) . " AND " . $maximum_price . " ";
				} elseif ($minimum_price != '') {
					$query .= "`price` >= " . $minimum_price . "  ";
				} elseif ($maximum_price != '') {
					$query .= "`price` <= " . $maximum_price . "  ";
				}
			}
			if ($category != '') {
				$query .=  ' AND `category_id` IN ("' . implode('","', $category) . '")';
			}
			if ($fabric != '') {
				$query .=  ' AND `fabric` IN ("' . implode('","', $fabric) . '")';
			}
			if ($length != '') {
				$query .=  ' AND `length` IN ("' . implode('","', $length) . '")';
			}
			if ($pattern != '') {
				$query .=  ' AND `pattern` IN ("' . implode('","', $pattern) . '")';
			}
		}  
// 		echo $query;
		$all_data = $this->CommonModal->runQuery($query);
		if (!empty($all_data) > 0) {
			return $all_data;
		} else {
			return false;
